...
// 作用：提供一个精美的图形化网页，用户在网页上点按钮，后台会自动帮你拼好复杂的命令行并执行。
const webHTML = `
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <button onclick="runCmd('auth login --email ' + v('email') + ' --password ' + v('password'))">登录 Apple ID</button>
        </div>
...
func webCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// [新增逻辑] 路由2：接收前端页面的指令，在后台自动执行命令行操作
